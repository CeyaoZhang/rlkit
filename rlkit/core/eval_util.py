...
def get_generic_path_information(paths, stat_prefix='')->OrderedDict:
    ...
    if len(paths) == 0:
        return statistics
    rewards = np.vstack([path["rewards"] for path in paths]) ## stack as (-1, 1)
    ...
    #             statistics.update(create_stats_ordered_dict(stat_prefix + k,final_ks,stat_prefix='{}/final/'.format(info_key),))
    #             statistics.update(create_stats_ordered_dict(stat_prefix + k,first_ks,stat_prefix='{}/initial/'.format(info_key),))
    #             statistics.update(create_stats_ordered_dict(stat_prefix + k,all_ks,stat_prefix='{}/'.format(info_key),))
